...
class KvotientPriority(models.Model):  # intern_kvotient_priority_nyintern
    application = models.ForeignKey(
        KvotientApplication, on_delete=models.CASCADE, related_name="priorities"
    )
...
class RoomOffer(models.Model):  # intern_kvotient_offer_nyintern (a room offered in a given month)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["room", "month"], name="uniq_room_offer_month")
        ]
...
class RoomConditionScore(models.Model):
    ...
    # Legacy stores one-or-more image path(s) per criterion as a raw string; kept as text for the
    # migration. Proper file storage (move into MEDIA, one row per image) is a feature-layer refinement.

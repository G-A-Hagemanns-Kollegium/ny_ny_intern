...
from oelkaelder.models import (
    Deposit, LogEntry, Product, PurchaseShare, Shopper, Transaction, TransactionItem, Warning,
)
    ...
    def handle(self, *args, **opts):
        ...
        for p in fetch_all("SELECT * FROM intern_oelkaelder_product"):
            ...
            Product.objects.update_or_create(
                id=p["productId"],
                defaults=dict(
                    name=(p["name"] or "").strip(), price_ore=max(p["current_price"] or 0, 0),
                    weight_price_ore=p["weight_price"] or None, price_steps=steps,
                    image=(p["imageurl"] or "").strip().lstrip("/"),
                    active=bool(p["active"]), highlighted=bool(p["highlighted"]),
                ),
            )

        # ---- shoppers (join saldo for active flag); skip shoppers without a migrated resident ----
        saldo_active = {r["shopperId"]: bool(r["active"]) for r in fetch_all("SELECT * FROM intern_oelkaelder_saldo")}
        shopper_ok = set()
        shopper_skipped = 0
        ...
        for d in fetch_all("SELECT * FROM intern_oelkaelder_deposit"):
            if d["shopperId"] not in shopper_ok:
                dep_skipped += 1
                continue
            deps.append(Deposit(id=d["ID"], shopper_id=d["shopperId"], amount_ore=max(d["amount"] or 0, 0),
                                created_at=d["time"] or timezone.now(), is_valid=bool(d["valid"])))
        ...
        for it in fetch_all("SELECT * FROM intern_oelkaelder_transaction_item"):
            ...
            items.append(TransactionItem(transaction_id=it["transactionId"], product_id=it["productId"],
                                         quantity=it["quantity"] or 1, price_ore=price))
        ...
        for tid, sids in buyers.items():
            ...
            for sid, amount in zip(sids, largest_remainder(item_total.get(tid, 0), len(sids))):
        ...
        for w in fetch_all("SELECT * FROM intern_oelkaelder_warnings"):
            Warning.objects.update_or_create(
                id=w["id"], defaults=dict(message=w["message"] or "", threshold_ore=w["amount"], active=bool(w["active"])),
            )

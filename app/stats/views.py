...
def stats(request):
    ...
    by_type = dict(
        Application.objects.values_list("type").annotate(c=Count("id")).values_list("type", "c")
    )
    ...
    heard_this_year = list(
        ...
        .values("heard_about_us").annotate(c=Count("id")).order_by("-c")
    )
    by_university = list(
        Application.objects.filter(type=Application.Type.TOUR, submitted_at__year=now.year)
        .exclude(university="")
        .values("university").annotate(c=Count("id")).order_by("-c")[:15]
    )
    visits = DailyVisitCount.objects.order_by("-date")[:30]
    return render(request, "statistik/stats.html", {
        "year": now.year,
